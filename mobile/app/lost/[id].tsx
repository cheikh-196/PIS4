...
import { LostReport } from '../../src/types/report';
...
export default function LostDetailScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await lostService.getById(id!);
  ...
  const handleResolve = () => {
    Alert.alert('Confirmer', 'Marquer ce signalement comme résolu ?', [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await lostService.updateStatus(id!, 'resolved');
    ...
    <ScrollView style={styles.container} contentContainerStyle={[styles.content, { maxWidth: contentMaxWidth, width: '100%', alignSelf: 'center' }]}>
      ...
      <View style={styles.body}>
        <View style={styles.header}>
          ...
          <Badge label={report.status === 'active' ? 'Actif' : 'Résolu'} variant={report.status === 'active' ? 'warning' : 'success'} />
        </View>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={styles.meta}>🗓 Perdu le {formatDate(report.lostDate)}</Text>
        </View>

        {report.reward > 0 && (
          <View style={styles.rewardBox}>
            <Text style={styles.rewardText}>💰 Récompense : {report.reward} UM</Text>
          </View>
        )}
          ...
          <>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {report.images.map((img, i) => (
                <Image key={i} source={{ uri: getImageUrl(img.url)! }} style={styles.thumb} />
              ))}
            </ScrollView>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  rewardText: { fontSize: FontSize.md, fontWeight: '600', color: Colors.warning },
  ...
  thumb: { width: 100, height: 100, borderRadius: BorderRadius.md, marginRight: Spacing.sm },
