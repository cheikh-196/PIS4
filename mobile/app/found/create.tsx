...
export default function CreateFoundScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (heldAt) formData.append('heldAt', heldAt);
      ...
      await foundService.create(formData);
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={[styles.scroll, { maxWidth: contentMaxWidth, width: '100%', alignSelf: 'center' }]} keyboardShouldPersistTaps="handled">
        <View style={styles.sectionHeader}>
          ...
          <Text style={[styles.section, { color: Colors.found }]}> Catégorie *</Text>
        </View>
        <CategorySelector selected={category} onSelect={setCategory} />

        <Input label="Titre *" value={title} onChangeText={setTitle} placeholder="Ex: iPhone 15 bleu" icon="text-outline" />
        <Input label="Description *" value={description} onChangeText={setDescription} placeholder="Décrivez l'objet..." multiline icon="document-text-outline" />
        ...
        <Input label="Date de découverte *" value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" icon="calendar-outline" />
        <Input label="Lieu de dépôt" value={heldAt} onChangeText={setHeldAt} placeholder="Ex: Accueil de la bibliothèque" icon="business-outline" />
